...
const updateMessageQueue = new Bull("updateContentType", REDIS);
...
const updateMsgContentType = async (discordId: string, expressId: string) => {
  if (!updateMessageQueue) return;
  const option = {
    attempts: 1000,
    backoff: 20000,
